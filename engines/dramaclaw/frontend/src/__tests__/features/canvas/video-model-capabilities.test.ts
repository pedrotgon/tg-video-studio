...
describe("HappyHorse 单图默认模式", () => {
  ...
  it("图片参考在 1~9 张图时都可用", () => {
    ...
    expect(source).toContain('if (images === 0) return "Conecte de 1 a 9 nós de imagem";');
    expect(source).toContain('if (images > 9) return "A referência aceita no máximo 9 imagens";');
  });
});
