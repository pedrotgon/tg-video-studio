...
const FOV_PRESETS = [
  { label: "Anúncio fisheye 160°", value: 160 },
  { label: "Anúncio lente estreita 120°", value: 120 },
  { label: "Anúncio padrão 70°", value: 70 },
  { label: "Anúncio telefoto 8°", value: 8 },
] as const;
...
export function PanoCaptureSurface({
  ...
  const capture = useCallback(async () => {
    ...
    const viewer = viewerRef.current;
    if (!canvas || !viewer) {
      setViewerError("O assistente de visualização 360° ainda não está pronto");
      return;
    }
    setIsCapturing(true);
    try {
  ...
  const contextLabels = [
    ...
    manifest.allowed_destinations.includes("beat_selected_background")
      ? "Salve o local-target selecionado"
      : "A entrada da cena só baixa, não grava o mais recente",
  ...
  return (
    <div className={cn("flex h-full min-h-0 flex-col bg-background", className)}>
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-border/60 px-4 py-2 text-xs">
        ...
        >
          {planetBackup ? "Sair do Asteroide" : "Perspectiva do Asteroide"}
        </Button>
        <Button variant="outline" size="sm" onClick={() => void copyCorrectionJson()}>
          Copiar parâmetro JSON
        </Button>
        {onSaveCorrection && (
          <Button
            variant="outline"
          ...
          >
            {isSavingCorrection ? "Poupança" : "Salvar correção"}
          </Button>
        )}
        ...
        <Button variant="outline" size="sm" onClick={resetCropFrame}>
          Redefinir caixa de captura de tela
        </Button>
      </div>
      <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-border/60 px-4 py-2 text-xs">
        ...
        <Button variant="outline" size="sm" onClick={setFrontYawFromView}>
          Atualmente definido para a frente
        </Button>
        ...
        {saveResult && (
          <div className="pointer-events-auto absolute bottom-3 right-3 z-10 max-w-[min(460px,calc(100%-24px))] rounded-lg border border-emerald-500/40 bg-background/90 p-3 text-xs shadow-lg backdrop-blur">
            <div className="font-medium text-emerald-600">
              Guardado {saveResult.anchor_id || "Toma uma foto"}
            </div>
            <div className="mt-1 break-all text-muted-foreground">
              {saveResult.rel_path || saveResult.path || "Caminho não retornado"}
            </div>
        ...
        {captureHistory.length > 0 && (
          <div className="pointer-events-auto absolute bottom-3 left-3 z-10 max-w-[min(560px,calc(100%-24px))] rounded-lg border border-border/70 bg-background/90 p-2 shadow-lg backdrop-blur">
            <div className="mb-2 flex items-center justify-between gap-3">
              <div className="text-xs font-medium text-foreground">Lista de capturas de tela</div>
              <Button variant="ghost" size="sm" onClick={clearCaptureHistory}>
                Lista de Capturas de Tela Vazias
              </Button>
            </div>
              ...
              {captureHistory.map((item) => (
                <div key={item.id} className="w-32 shrink-0 overflow-hidden rounded-md border border-border bg-card">
                  ...
                  <div className="space-y-0.5 p-2 text-[11px] leading-tight">
                    <div className="font-medium text-foreground">Toma uma foto {item.id}</div>
                    ...
                    >
                      Baixar captura de tela {item.id}
                    </a>
                  </div>
                </div>
              ))}
