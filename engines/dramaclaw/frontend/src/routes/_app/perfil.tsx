...
import { CopyReview, reviewLabels } from "@/components/profile/CopyReview";
import { ClientEditor } from "@/components/profile/ClientEditor";
...
type CopyItem = {
  ...
  caption?: string;
  text: string;
  status: "draft" | "adjusted" | "approved" | "recorded" | "published" | "rejected";
...
<p className="text-xs text-[#031A26]/85 leading-relaxed">
        ...
        {tab === "memoria" && <><ClientEditor profile={profile} endpoint={endpoint} onSaved={() => { void query.refetch(); }} /><EvidenceMemory posts={posts} transcripts={data?.transcripts || []} blocks={data?.copy_blocks || []} onReference={id => { setSourcePostId(id); setSelectedKeywords([/MUNDOFIT/i.test(posts.find(p=>p.id===id)?.caption || '')?'MUNDOFIT':'QUERO']); setTab('copies'); }} /></>}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {copies.map((copy, index) => (
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-xs text-gray-400 font-mono">
                            {reviewLabels[copy.status] || copy.status}
                          </span>
                        </div>
                        ...
                        {copy.hook_spoken && !copy.text.trim().startsWith(copy.hook_spoken.trim()) && (
                        ...
                        <div className="text-sm text-[#031A26] leading-relaxed mb-4 space-y-3">
                      ...
                      <CopyReview copy={copy} endpoint={endpoint} onSaved={() => { void query.refetch(); }} />
                      <div className="border-t border-[#DCE1E3] pt-3 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => copyToClipboard(formatCadence(copy.text).join("\n\n"), copy.id)}
