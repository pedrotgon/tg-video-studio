...
export const App: React.FC = () => {
  ...
  const changeTab = (tab: TabType) => {
    ...
    setActiveTab(tab);
    window.history.replaceState(null, '', tab === 'animation' ? '#animation' : tab === 'complex' ? '#creative' : window.location.pathname + window.location.search);
    setActiveJob(null);
  };
  ...
  return (
    <div className="studio-shell">
      ...
      <div className="studio-main">
        {activeTab === 'complex' ? <Suspense fallback={<p className="p-6">Abrindo projetos…</p>}><CreativeHub /></Suspense> : activeTab === 'animation' ? <Suspense fallback={<p role="status" className="p-6">Preparando o palco…</p>}><StopMotionStudio /></Suspense> : <>
